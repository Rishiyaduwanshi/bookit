export default [

  {
    _id: 3,
    name: "Swiss Alps Escape",
    imgSrc: "x",
    location: "Switzerland",
    description:
      "A breathtaking journey through snowy peaks, charming villages, and scenic railways.",
    price: 1450,
  },
  {
    _id: 4,
    name: "Tokyo Highlights",
    imgSrc: "",
    location: "Japan",
    description:
      "Dive into Japan’s futuristic capital filled with culture, cuisine, and neon vibes.",
    price: 1100,
  },
  {
    _id: 5,
    name: "Paris City Lights",
    imgSrc: "",
    location: "France",
    description:
      "Romance, art, and architecture await in the city of love — Paris.",
    price: 1350,
  },
  {
    _id: 6,
    name: "Maldives Paradise",
    imgSrc: "",
    location: "Maldives",
    description:
      "Crystal-clear waters and overwater villas for the ultimate tropical escape.",
    price: 2200,
  },
  {
    _id: 7,
    name: "New York Explorer",
    imgSrc: "",
    location: "USA",
    description:
      "Skyscrapers, Broadway, and bustling streets — the city that never sleeps.",
    price: 950,
  },
  {
    _id: 8,
    name: "Cappadocia Balloons",
    imgSrc: "",
    location: "Turkey",
    description:
      "Fly high among the fairy chimneys in a hot-air balloon at sunrise.",
    price: 870,
  },
  {
    _id: 9,
    name: "Dubai Luxury Trip",
    imgSrc: "",
    location: "UAE",
    description:
      "Skyscrapers, desert safaris, and gold souks — the heart of modern luxury.",
    price: 1600,
  },
  {
    _id: 10,
    name: "London Classics",
    imgSrc:
      "",
    location: "United Kingdom",
    description:
      "Explore royal palaces, historic pubs, and vibrant markets in London.",
    price: 1200,
  },
  {
    _id: 11,
    name: "Iceland Expedition",
    imgSrc: "",
    location: "Iceland",
    description:
      "Chase northern lights and witness dramatic landscapes of fire and ice.",
    price: 1750,
  },
  {
    _id: 12,
    name: "Rome Heritage Walk",
    imgSrc: "",
    location: "Italy",
    description:
      "Ancient ruins, timeless art, and authentic Italian cuisine await in Rome.",
    price: 1150,
  },
  {
    _id: 13,
    name: "Bangkok Street Life",
    imgSrc: "",
    location: "Thailand",
    description:
      "Vibrant streets, delicious street food, and ornate temples in Bangkok.",
    price: 780,
  },
  {
    _id: 14,
    name: "Sydney Harbour Escape",
    imgSrc: "",
    location: "Australia",
    description:
      "See the Sydney Opera House, surf at Bondi, and soak in Australian vibes.",
    price: 1320,
  },
  {
    _id: 15,
    name: "Machu Picchu Hike",
    imgSrc: "",
    location: "Peru",
    description:
      "Trek through the Andes to discover the lost city of the Incas.",
    price: 1800,
  },
  {
    _id: 16,
    name: "Cairo & The Pyramids",
    imgSrc: "",
    location: "Egypt",
    description:
      "Journey into ancient history with pyramids, the Nile, and bustling bazaars.",
    price: 990,
  },
  {
    _id: 17,
    name: "Los Angeles Dream Tour",
    imgSrc: "https://images.unsplash.com/photo-1500048993953-d23a436266cf",
    location: "USA",
    description:
      "Hollywood, beaches, and sunshine — explore the glam side of LA.",
    price: 1100,
  },
  {
    _id: 18,
    name: "Seoul City Break",
    imgSrc: "https://images.unsplash.com/photo-1743906487913-1919e4c71537",
    location: "South Korea",
    description:
      "Trendy cafes, K-pop culture, and modern architecture meet ancient palaces.",
    price: 1050,
  },
  {
    _id: 19,
    name: "Rio Carnival Experience",
    imgSrc: "https://images.unsplash.com/photo-1716942739315-1b7aad1f30b7",
    location: "Brazil",
    description:
      "Dance through the streets during the world’s most vibrant carnival.",
    price: 1250,
  },
  {
    _id: 20,
    name: "Cape Town Adventure",
    imgSrc: "https://images.unsplash.com/photo-1505678261036-a3fcc5e884ee",
    location: "South Africa",
    description:
      "Hike Table Mountain, enjoy coastal drives, and explore rich local culture.",
    price: 1180,
  },
];
